  ...
  <section class="app-container calendar-list-container">
    ...
    >
      ...
      <div v-else>
        ...
        <el-form-item label="支付宝公钥(PUBLIC_KEY)" prop="publicKey">
          <el-input v-model="moduleInfo.publicKey"></el-input>
        </el-form-item>
        <el-form-item label="应用私钥(PRIVATE_KEY)" prop="privateKey">
...
<script>
  import {addModuleFun} from "@/api/activity"
    ...
    methods: {
      submitForm(formName) {
        this.$refs[formName].validate(valid => {
          if (valid) {
            addModuleFun(this.moduleInfo).then(res => {
              if (res.data.success) {
                this.$message({
                  showClose: true,
                  message: "新增模板成功",
