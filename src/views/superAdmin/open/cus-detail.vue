<template>
  <section class="p2">
    ...
    <el-tabs v-model="activeName">
      ...
      <el-tab-pane v-if="type===2" label="基本资料" name="second">
        <ul>
          ...
          <li v-if="customInfo.type==='代理'">
            <h5>代理详情：</h5>
            <div v-for="item in customInfo.agent_info">
              <span>{{item.brand_name}}--{{item.agent_provice}}{{item.agent_city}}{{item.agent_county}}</span>
            </div>
          </li>
        </ul>
      </el-tab-pane>
      <el-tab-pane v-if="type===2" label="客户订单" name="third">
        <el-table :data="orderList" stripe style="width: 100%">
          ...
          <el-table-column label="下单日期">
            <template slot-scope="scope">
              {{$moment(scope.row.cdate*1000).format('YYYY-MM-DD')}}
            </template>
          </el-table-column>
    ...
    <el-dialog title="查看跟进动态" :visible="checkFollowRecord" :show-close="false">
      <ul class="follow-wrapper" v-if="otherFollowInfo.length">
        <li v-for="(item,index) in otherFollowInfo" :key="index" class="follow-item">
          <div>
            <img src="../../../../static/img/wechat.png" style="width:32px" alt="" v-if="item.follow_type==='微信'">
            <img src="../../../../static/img/tel.png" style="width:32px" alt="" v-if="item.follow_type==='电话'">
            <img src="../../../../static/img/order.png" style="width:32px" alt="" v-if="item.follow_type==='订单'">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="checkFollowRecord = false">确定</el-button>
      </div>
    </el-dialog>
    <!--添加订单-->
  ...
  export default {
    ...
    data() {
      return {
        ...
        activeName: this.$route.query.type === 1 ? 'fifth' : 'first',
        cusId: this.$route.query.id,
        customInfo: {},//客户信息
        orderList: [],//订单列表
        trandInfo: {},//交易信息
        trendList: {},//跟进动态
        otherTrendList: {},//别人的跟进动态
        followRecordInfo: {
          cdate: new Date()
        },//添加跟进动态
    ...
    methods: {
      ...
      getAllBusiness() {
        let params = {
          puid: this.customInfo.puid || this.$route.query.puid,
        };
