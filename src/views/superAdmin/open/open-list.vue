    ...
    <div class="app-container calendar-list-container client-list">
        ...
        <el-table :data="txList" stripe style="width: 100%" class="mt2" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="cdate" label="创建时间">
                <template slot-scope="scope">
                    <span>{{$moment(scope.row.cdate).format('YYYY-MM-DD')}}</span>
                </template>
            </el-table-column>
...
<script>
	import {openCustomerList, exportCustomerFun, delOpenCustomer, changeToMyCustomerFun, releaseCustomerFun} from '@/api/activity'
