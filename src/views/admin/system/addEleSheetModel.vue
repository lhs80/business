<template>
  <div class="app-container calendar-list-container addStaff-list">
    ...
    <el-form :model="sheetInfo" :rules="rules" ref="sheetInfo" label-width="150px">
      ...
      <el-form-item>
        <el-button type="primary" @click="addEleSheetModel('sheetInfo')">提交</el-button>
...
<script>
  import {allExpressFun, addEleSheetModelFun} from "@/api/activity"
  ...
  export default {
    ...
    methods: {
      ...
      addEleSheetModel(formName) {
        this.$refs[formName].validate(valid => {
          if (valid) {
            addEleSheetModelFun(this.sheetInfo).then(res => {
              if (res.data.success) {
                this.$message({
                  message: '添加模板成功！',
                  type: 'success',
