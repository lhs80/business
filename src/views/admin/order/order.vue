    ...
    <div class="app-container calendar-list-container order-list">
        ...
        <aside class="order-panel mt2" v-for="(item,index) in orderList" :key="index">
            <el-row class="bg-grey h6 prl2 ptb1">
                <el-col :span="12"><b>{{timestampToTime(item.cdate)}}</b> <span
                        class="text-muted prl1">订单编号：{{item.order_id}}</span></el-col>
                <el-col :span="12" class="text-right text-muted">
                    <el-tooltip class="item" effect="dark" :content="item.message?item.message:'无'" placement="top">
                        <span>备注</span>
                    </el-tooltip>
                </el-col>
            </el-row>
            <el-row class="order-panel-content" type="flex" align="middle">
                ...
                <el-col :span="3" class="text-center h6">
                    <div>
                        ...
                        <el-button type="primary" size="small" @click="showUploadPanel(item)" v-if="item.order_status===2">修改订单</el-button>
                        <!--<el-button type="primary" size="small" @click="showSendPanel(item)" v-if="item.order_status===2">确认发货</el-button>-->
        ...
        <!--确认发货弹框-->
        <div tabindex="-1" role="dialog" aria-modal="true" aria-label="提示" class="el-message-box__wrapper" style="z-index: 99;"
             v-show="isShowSendPanel">
            <div class="el-message-box el-message-box--center">
                <div class="bg-grey">
                    <el-row class="prl2">
                ...
                <div class="el-message-box__content">
                    ...
                    >
                        <el-form-item class="text-left">
                            <h5>收货人：{{curOrder.receipt_person}}{{curOrder.receipt_phone}}</h5>
                            <h5>地址：{{curOrder.address}}</h5>
                        </el-form-item>
                        <el-form-item label="快递公司" prop="postName">
                            <el-select v-model="delive.postName" placeholder="请选择">
                                <el-option
                        ...
                        <el-form-item label="快递单号" prop="postCode">
                            <el-input v-model="delive.postCode"></el-input>
                        </el-form-item>
                    </el-form>
                </div>
                <div class="el-message-box__btns">
                    <el-button type="primary" @click="sendProduct('postForm')">确认发货</el-button>
                    <el-button @click="isShowSendPanel=false">关闭</el-button>
                </div>
            </div>
        </div>
        <div class="v-modal" tabindex="0" style="z-index: 80;" v-if="isShowSendPanel"></div>
        <!--修改订单-->
...
<script>
	import {orderListFun, getOrderDetailFun, deliveExpressFun, allExpressFun} from '@/api/activity'
				...
				this.$refs[formName].validate((valid, object) => {
					if (valid) {
						let params = {
							...
							ShipperCode: this.delive.postName,
							LogisticCode: this.delive.postCode
						};
						deliveExpressFun(params).then(res => {
							if (res.data.success) {
								this.$message({
									message: '发货成功！',
									type: 'success',
